...
void verifica_var(char linha[])//função para verificar o comando var
{
  if(linha[4]=='i' && linha[5]=='n' && linha[6]=='t' && linha[7]==' ')//Váriavel como tipo inteiro
    ...
    else if(linha[4]=='c' && linha[5]=='h' && linha[6]=='a' && linha[7]=='r' && linha[8]==' ')
    {
        ...
        int chave_char=0;
        for(int i = 9; i<strlen(linha)-2; i++)
        {
            if(linha[i]=='[')
            {
                chave_char+=1;
...
int main()
{
    ...
    int verificador_bloco_condicao = 0;
    ...
    while(!feof(fp))
    {
        ...
        if((strncmp(linha, "!start", 5)==0))
        {
            verificador_start+=1;
            if(verificador_start>1)
            {
        ...
        if(strncmp(linha, "func void main()", 16)==0 || strncmp(linha, "func void main ()", 17)==0)
        {
            if(verificador_main>1)
            {
